...
var methods = {
	...
	"Argon2id-16Mx1": { func: function(site, masterpw, generation, len) { return argon2.hash({distPath: 'argon2-browser/dist', pass: masterpw, salt: site + "#" + generation + "#" + ARGON2_PEPPER, time: 1, mem: 16384, hashLen: Math.floor((len*1+1) * 3 / 4), parallelism: 1, type: argon2.ArgonType.Argon2id}).then(function(hash) { return CryptoJS.enc.Base64.stringify(CryptoJS.enc.Hex.parse(hash.hashHex)); }); }, command: "echo \"$masterpwd\" | argon2 \"$site#$generation#" + ARGON2_PEPPER + "\" -id -r -t 1 -m 14 -p 1 -l $(((len+1)*3/4)) | xxd -r -p | openssl base64 -e | cut -c 1-$len" },
};
