...
var methods = {
	...
	"Argon2id-12x8M": { func: function(site, masterpw, generation, len) { return argon2.hash({distPath: 'argon2-browser/dist', pass: masterpw, salt: site + "#" + generation + "#" + ARGON2_PEPPER, time: 12, mem: 8192, hashLen: Math.floor((len*1+1) * 3 / 4), parallelism: 1, type: argon2.ArgonType.Argon2id}).then(function(hash) { return CryptoJS.enc.Base64.stringify(CryptoJS.enc.Hex.parse(hash.hashHex)); }); }, command: "echo \"$masterpwd\" | argon2 \"$site#$generation#" + ARGON2_PEPPER + "\" -id -r -t 12 -m 13 -p 1 -l $(((len+1)*3/4)) | xxd -r -p | openssl base64 -e | cut -c 1-$len" },
	"Argon2id-3x64M": { func: function(site, masterpw, generation, len) { return argon2.hash({distPath: 'argon2-browser/dist', pass: masterpw, salt: site + "#" + generation + "#" + ARGON2_PEPPER, time: 3, mem: 65536, hashLen: Math.floor((len*1+1) * 3 / 4), parallelism: 1, type: argon2.ArgonType.Argon2id}).then(function(hash) { return CryptoJS.enc.Base64.stringify(CryptoJS.enc.Hex.parse(hash.hashHex)); }); }, command: "echo \"$masterpwd\" | argon2 \"$site#$generation#" + ARGON2_PEPPER + "\" -id -r -t 3 -m 16 -p 1 -l $(((len+1)*3/4)) | xxd -r -p | openssl base64 -e | cut -c 1-$len" },
};
