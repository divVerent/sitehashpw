let settings_sync = {"method": "HMAC-SHA-256", "len": 12, "overrides": {}, "generation": 1};
let settings_local = {"masterpw": "", "oldmasterpw": ""};
let settings_nosave = {"last_site": "", "masterpw_temp": "", "oldmasterpw_temp": ""};
...
function fix_override(sitename, override) {
	let site_settings = get_default_settings(sitename);
...
function fix_overrides(v) {
	...
	for (let k in v) {
		let override = v[k];
		let site_settings = fix_override(k, override);
...
function get_sitepw(url, allow_write) {
	...
	const pw_tempkey = site_settings.use_old_password ? "oldmasterpw_temp" : "masterpw_temp";
			...
			chrome.runtime.onMessage.addListener(
				(request, sender, sendResponse) => {
					if (request.sitepw_get_settings != null) {
						let out = {};
						for (let i = 0; i < request.sitepw_get_settings.length; ++i) {
